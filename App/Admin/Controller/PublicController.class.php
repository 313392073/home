    ...
    public function responseMss() {
        ...
        if(strtolower($postObj->MsgType) == 'event') {
            ...
            if(strtolower($postObj->Event) == 'subscribe') {//扫码首次关注后
                $scene_id = substr(trim($postObj->EventKey),8);//agent_id场景值
                    if($postObj->EventKey){
                    ...
                    if($curl_res['time'] == 0){
                        ...
                        $time = date('Y-m-d h:i:s',$curl_res['time']);
                    }
                    ...
                    }else if($curl_res['agentId'] == $re['game_id']) {
                        $content = "大渝棋牌，人人精彩🔥\n加入茶楼成功！\n你已经加入了“".$re['tea_name']."”,加入时间“".$time."”,您的游戏ID为：“".$curl_res['uid']."”\n赶快打开\n<a href='http://wx.baiyy.com/dayugm/wechat/index.php/Home/Login/login.html'>点击下载《大渝棋牌》</a>，进行游戏吧！";
                    }else{
                        $res = $uOb->where("game_id='%s'",$curl_res['agentId'])->find();
                        $content = "大渝棋牌，人人精彩🔥\n加入茶楼失败！\n你已经加入了“".$res['tea_name']."”,加入时间“".$time."”,您的游戏ID为：“".$curl_res['uid']."”\n加入茶楼失败🌷\n<a href='http://wx.baiyy.com/dayugm/wechat/index.php/Home/Login/login.html'>点击下载《大渝棋牌》🌷</a>";
                    }
                }else{
                        $content = "大渝棋牌，全民娱乐🔥\n线上茶馆，轻松创业🍭\n感谢关注大渝棋牌微信公众号，恭喜你已经找到了最好玩的约牌组织!热门活动及游戏咨询请添加客服妹纸微信：dyqp001或dyqp002🍭\n<a href='http://wx.baiyy.com/dayugm/wechat/index.php/Home/Login/login.html'>点击下载《大渝棋牌》</a>";
                }
                ...
                //扫码再次扫码进入
                $scene_id = $postObj->EventKey;//agent_id场景值
                $re = $uOb->where("agent_id='%s'",$scene_id)->find();
                $time = '';
                $url=C('API_URL').'/?act=queryAgentId&openid='.$unionId.'&agentId='.$re['game_id'];
                $curl_res=get_curl($url);
                ...
                $content = "大渝棋牌，全民娱乐\n加入茶楼失败！！！您已经加入了“".$res['tea_name']."”加入时间“".$time."”,您的游戏ID为：“".$curl_res['uid']."\n加入茶楼失败🌹”\n<a href='http://www.wx.baiyy.com'>点击下载《大渝棋牌》</a>";
                $this->replyText($toUser,$fromUser,$msgType,$content);
...
//                            'key'=>'item2'
                        ...
                        array(
                            'name'=>urlencode('下载游戏'),
                            'type'=>'view',
                            'url'=>'http://wx.baiyy.com/dayugm/wechat/index.php/Home/Look/userAgree'
                        ),//第二个二级菜单
                    )
                ),
